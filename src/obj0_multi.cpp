...
// [[Rcpp::export]]
double obj0cpp_multi(arma::vec w, arma::vec beta, arma::vec mu, double rho,
                     double alpha, double rho1,
                     arma::mat X, arma::mat Y, arma::mat white_coefs, int idx_intcpt,
                     int n_resps, double n_obs, double n_nodes, int n_coefs,
  ...
  arma::mat Mu_multi = exp(H);

  arma::mat Deno = log(Mu_multi * trans(Sr));
  arma::mat Nume = (Y % H) * trans(Sr);

  double loss = -(1.0/n_obs) * accu(Nume - Deno) ;
